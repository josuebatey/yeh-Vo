...
import {
  Copy, QrCode, Volume2,
  Share, ExternalLink, CheckCircle,
  Smartphone, Download
} from 'lucide-react'
...
export function ReceivePayment() {
  ...
  const updateQRCodeData = useCallback(() => {
    ...
    const params = new URLSearchParams({ action: 'send', to: wallet.address })
    if (amount && parseFloat(amount) > 0) params.set('amount', amount)
    if (note.trim()) params.set('note', note.trim())

    qrCodeInstanceRef.current.update({
      data: `${protocol}//${host}/send?${params.toString()}`
    })
  }, [wallet?.address, amount, note])

  // Re-run when amount or note changes
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    try {
      qrCodeInstanceRef.current.update({ width: size, height: size })
    } catch (e) {
      console.error('Failed to update QR size:', e)
    }
  }, [isMobile])

  // QR container ref setup & cleanup
  const qrCodeRef = useCallback((node: HTMLDivElement | null) => {
    if (prevContainerRef.current && qrCodeInstanceRef.current) {
    ...
    prevContainerRef.current = node
  }, [wallet?.address, isMobile, updateQRCodeData])
  ...
  const copyPaymentLink = async () => {
    ...
    if (amount && parseFloat(amount) > 0) params.set('amount', amount)
    if (note.trim()) params.set('note', note.trim())
    const deepLinkUrl = `${protocol}//${host}/send?${params.toString()}`
    try {
  ...
  const sharePaymentRequest = async () => {
    ...
    const shareData = { title: 'VoicePay Payment Request', text: shareText, url: deepLinkUrl }

    try {
      if (navigator.share && isMobile && (navigator.canShare?.(shareData) ?? true)) {
  ...
  const downloadQRCode = () => {
    if (qrCodeInstanceRef.current && qrGenerated) {
      qrCodeInstanceRef.current.download({ name: `voicepay-qr-${Date.now()}`, extension: 'png' })
        ...
        <Card className="w-full">
          <CardHeader className="pb-4">
            <CardTitle className="flex items-center gap-2 text-base md:text-lg">
              ...
              {qrGenerated && <CheckCircle className="h-4 w-4 text-green-500 flex-shrink-0" />}
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4 w-full">
            <div className="flex justify-center w-full">
              <div
                ...
                style={{ width: isMobile ? '240px' : '280px', height: isMobile ? '240px' : '280px' }}
              >
                {!qrGenerated && (
            ...
            <div className="text-center space-y-3 w-full">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 w-full">
                <Button onClick={copyPaymentLink} variant="outline" size="sm" className="w-full text-xs">
                  <Copy className="mr-1 h-3 w-3 flex-shrink-0" /> Copy Link
                </Button>
                <Button onClick={sharePaymentRequest} size="sm" className="w-full text-xs">
                  <Share className="mr-1 h-3 w-3 flex-shrink-0" /> Share
                </Button>
                <Button onClick={downloadQRCode} variant="outline" size="sm" className="w-full text-xs" disabled={!qrGenerated}>
                  <Download className="mr-1 h-3 w-3 flex-shrink-0" /> Download
                </Button>
                <Button onClick={refreshQRCode} variant="outline" size="sm" className="w-full text-xs">
                  <QrCode className="mr-1 h-3 w-3 flex-shrink-0" /> Refresh
                </Button>
              </div>
            </div>
          </CardContent>
