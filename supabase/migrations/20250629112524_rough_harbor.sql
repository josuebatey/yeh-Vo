/*
  # Fix wallet RLS policies and constraints

  1. Security Updates
    - Update RLS policy for wallets table to allow proper INSERT operations
    - Ensure users can only insert wallets for themselves
    - Add proper constraint to prevent multiple wallets per user

  2. Data Integrity
    - Add unique constraint on user_id to prevent multiple wallets per user
    - Update existing policies to be more specific

  3. Policy Changes
    - Replace generic "Users can manage own wallet" with specific policies
    - Add separate INSERT, SELECT, UPDATE, DELETE policies for better control
*/

-- First, drop the existing generic policy
DROP POLICY IF EXISTS "Users can manage own wallet" ON wallets;

-- Add unique constraint to prevent multiple wallets per user
DO $$
BEGIN
  IF NOT EXISTS (
